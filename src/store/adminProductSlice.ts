...
export const {setStatus,setProduct,setProducts,addProductToProducts}=productSlice.actions
...
export function fetchProductAdmin(id:string){
    return async function fetchProductAdminThunk(dispatch:AppDispatch,getState:()=>RootState){
        ...
        const productExists = store.adminProducts.products.find((product:IProduct)=>product.id == id) 
