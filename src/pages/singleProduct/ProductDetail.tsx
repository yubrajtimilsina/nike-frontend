...
const ProductDetail = () => {
  ...
  const availableSizes =
    product?.sizes?.length > 0 ? product?.sizes : ["No sizes available"];
  const availableColors =
    product?.colors?.length > 0 ? product?.colors : ["No colors available"];
              ...
              <div className="flex flex-wrap gap-2">
                {availableColors.map((color) => (
                  <button
                    key={color}
                    onClick={() =>
                      color !== "No colors available" && setSelectedColor(color)
                    }
                    className={`px-4 py-2 border rounded-md ${
                      selectedColor === color
                        ? "bg-indigo-600 text-white border-indigo-600"
