...
function Checkout(){
    ...
    const total = data.reduce((total,item)=>item.Shoe.price * item.quantity + total,0)
    ...
    const handleSubmit = async (e:FormEvent<HTMLFormElement>)=>{
        e.preventDefault()
        const productData =  data.length > 0 ? data.map((item)=>{
            return {
        ...
        data.length > 0 ? data.map((item)=>{
            return ( 
                ...
                <div className="w-32 h-28 max-lg:w-24 max-lg:h-24 flex p-3 shrink-0 bg-gray-200 rounded-md">
                  <img src={`http://localhost:3000/${item.Shoe?.images}`} className="w-full object-contain" />
                </div>
                <div className="w-full">
                  <h3 className="text-sm lg:text-base text-gray-800">{item.Shoe.name}</h3>
                  <ul className="text-xs text-gray-800 space-y-1 mt-3">
                    ...
                    <li className="flex flex-wrap gap-4">Total Price <span className="ml-auto">Rs.{item?.Shoe?.price}</span></li>
                  </ul>
                </div>
              </div>
            )
