        ...
        orderDetails.length > 0 && orderDetails.map((od)=>{
          return ( 
            ...
            <div className="pb-4 md:pb-8 w-full md:w-40">
              <img className="w-full hidden md:block" src={`http://localhost:5001/${od?.Shoe?.images}`} alt="dress" />
          
            </div>
            <div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
              <div className="w-full flex flex-col justify-start items-start space-y-8">
                <h3 className="text-xl dark:text-white xl:text-2xl font-semibold leading-6 text-gray-800">{od?.Shoe?.name}</h3>
                <div className="flex justify-start items-start flex-col space-y-2">
                  <p className="text-sm dark:text-white leading-none text-gray-800"><span className="dark:text-gray-400 text-gray-300">Category: </span> {od?.Shoe?.Category?.categoryName}</p>

                </div>
              </div>
              <div className="flex justify-between space-x-8 items-start w-full">
                <p className="text-base dark:text-white xl:text-lg leading-6">Rs.{od.Shoe.price} </p>
                <p className="text-base dark:text-white xl:text-lg leading-6 text-gray-800">{od.quantity}</p>
                <p className="text-base dark:text-white xl:text-lg font-semibold leading-6 text-gray-800">Rs.{od.quantity * od.Shoe.price}</p>
              </div>
            </div>
          </div>
          )
